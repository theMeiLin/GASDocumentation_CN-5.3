# GASDocumentation
* 我对虚幻引擎 5 的 `GameplayAbilitySystem` 插件 (GAS) 的理解以及一个简单的多人示例项目。这不是官方文档，本项目和我本人都与 Epic Games 无关。 
* 我不保证这些信息的准确性。 本文档的目的是解释 GAS 中的主要概念和类，并根据我的使用经验提供一些补充说明。
* 社区中的用户对 `GAS` 有很多 "部落知识(组织的集体智慧)"，我希望在此分享我的所有知识。 示例项目和文档目前使用的是虚幻引擎 5.3 (UE5)。
* 本文档有适用于虚幻引擎旧版本的分支，但是这些分支已不再受支持，并且可能存在错误或信息过时。请使用与您的引擎版本相匹配的分支。

[GASShooter](https://github.com/tranek/GASShooter) 是一个姊妹示例项目，展示了用于多人 FPS/TPS 的 GAS 高级技术。
...
>游戏能力系统是一个高度灵活的框架，用于构建您可能喜欢的能力和属性 RPG或MOBA游戏中找到。您可以为游戏中的角色构建动作或被动能力以供使用，
>由于这些行动而可以增加或消耗各种属性的状态效果，请实施“冷却” 计时器或资源成本来规范这些操作的使用，更改能力级别及其在每个级别的效果，
>激活粒子或声音效果等。简单来说，这个系统可以帮助你设计、实施、 并有效地将游戏中的能力联网，既简单如跳跃，也复杂如您最喜欢的角色
>任何现代RPG或MOBA游戏中设置的能力。

GameplayAbilitySystem插件由Epic Games开发，并附带虚幻引擎。它已在 AAA 商业游戏中进行了实战测试，例如 Paragon 和 Fortnite 等。

该插件在单人和多人游戏中提供了开箱即用的解决方案，适用于：
* 实现基于等级的角色能力或技能，并具有可选的成本和冷却时间 ([GameplayAbilities](#concepts-ga))
* 操纵数值属于演员的 `Attributes`  ([Attributes](#concepts-a))
* 将状态效果应用于演员 ([GameplayEffects](#concepts-ge))
* 将 `GameplayTags` 应用与Actors ([GameplayTags](#concepts-gt))
* 生成视觉或声音效果 ([GameplayCues](#concepts-gc))
* 复制上述所有内容

在多人游戏中，GAS 支持 [客户端预测](#concepts-p):
* 技能激活
* 播放动画蒙太奇
* 对 `Attributes` 的更改
* 应用 `GameplayTags`
...
**必须在 C++ 中设置 GAS**, 但是 `GameplayAbilities` 和 `GameplayEffects` 可以在蓝图中创建。

GAS 的当前问题:
* `GameplayEffect` 延迟协调（无法预测技能冷却时间，导致延迟较高的玩家与延迟较低的玩家相比，低冷却技能的射速较低）。 
* 无法预知 `GameplayEffects` 的移除。然而，我们可以预测添加具有相反效果的 `GameplayEffects`，从而达到移除的效果。但这并不总是合适或可行，仍旧是一个存在的问题
* 缺少样板模板、多人游戏示例和文档。希望这对您有所帮助！

**[⬆ Back to Top](#table-of-contents)**

<a name="sp"></a>
## 2. 示例项目
本文档中包含一个多人第三人称射击游戏示例项目，面向刚接触GameplayAbilitySystem插件但不熟悉虚幻引擎的人。用户应了解 UE 中的 C++、蓝图、
UMG、复制和其他中间主题。此项目提供了一个示例，说明如何设置一个基本的第三人称射击多人游戏就绪项目，其中“PlayerState”类上的
`AbilitySystemComponent` （`ASC`） 用于玩家/AI 控制的英雄，`Character` 类上的 `ASC` 用于 AI 控制的仆从。

目标是保持这个项目的简单性，同时展示 GAS 基础知识，并用注释良好的代码演示一些常用的功能。由于其以初学者为重点，
该项目不显示 [预测投射物](#concepts-p-spawn) 等高级主题.
...
* 死亡和重生
* 从服务器上的技能中生成演员（投射物）
* 预测性地改变本地玩家在瞄准和疾跑时的速度
* 持续消耗耐力以进行疾跑
* 使用法力值施放技能
* 被动技能
* 叠加 `GameplayEffects`
* 定位 actors
* `GameplayAbilities` 在蓝图中创建
* `GameplayAbilities` 在 C++ 中创建
* 实例化每个 `Actor` `GameplayAbilities`
* 非实例化 `GameplayAbilities` (Jump)
* 静态 `GameplayCues` （开火枪械投射物撞击粒子效果）
* Actor `GameplayCues` (疾跑与眩晕粒子效果)

The hero class has the following abilities:

| 能力                    | 输入绑定          | 预测  | C++ / Blueprint | 描述                                                                                                                                                                  |
| -------------------------- | ------------------- | ---------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Jump                       | 空格键           | Yes        | C++             | 使英雄跳跃。                                                                                                                                                        |
| Gun                        | 鼠标左键   | No         | C++             | 从英雄的枪械发射投射物。动画进行了预测处理，但投射物的实际发射则没有预测                                                                               |
| Aim Down Sights            | 鼠标右键  | Yes        | Blueprint       | 按下按钮期间，英雄将会减速行走，并且相机将拉近，以便用枪械进行更精确的射击                                                   |
| Sprint                     | Left Shift          | Yes        | Blueprint       | 按下按钮时，英雄会快速奔跑，同时消耗体力（stamina）                                                                                                       |
| Forward Dash               | Q                   | Yes        | Blueprint       | 英雄向前猛冲，以消耗体力（stamina）为代价。                                                                                                                            |
| Passive Armor Stacks       | 被动             | No         | Blueprint       | 每4秒，英雄获得一层护甲，最多可叠加至4层。受到伤害时会移除一层护甲                                                   |
| Meteor                     | R                   | No         | Blueprint       | 玩家瞄准一个地点，向敌人投下一颗流星，造成伤害并击晕他们。目标是预测的，而生成流星则不是。                     |

无论 `GameplayAbilities`（游戏能力）是用C++还是Blueprint（蓝图）创建的都没有关系。这里混合使用了两种方式来展示如何用每种语言实现它们。

小兵（Minions）没有预设的 `GameplayAbilities`。红色小兵具有更高的生命值回复速度，而蓝色小兵则初始生命值更高。

在命名 `GameplayAbility` 时，我使用了后缀_BP来表示该 `GameplayAbility` 的逻辑是在Blueprint中创建的。如果没有这个后缀，则意味着逻辑是在C++中编写的。
...
1. 编辑 `YourProjectName.Build.cs` 文件，向 `PrivateDependencyModuleNames` 中添加 `GameplayAbilities`, `GameplayTags`, `GameplayTasks`。
1. 刷新/重新生成 Visual Studio 项目文件
1. 从4.24版本开始, 为了使用[`TargetData`](#concepts-targeting-data)，必须调用 `UAbilitySystemGlobals::Get().InitGlobalData()` 。 
2. 示例项目在 `UAssetManager::StartInitialLoading()`中完成了这一操作。更多详情，请参见 [`InitGlobalData()`](#concepts-asg-initglobaldata)。

以上就是启用GAS（Gameplay Ability System，游戏能力系统）所需的所有步骤。接下来，向你的Character或PlayerState添加一个[`ASC`](#concepts-asc) 
和 [`AttributeSet`](#concepts-as)，然后就可以开始创建[`GameplayAbilities`](#concepts-ga) and [`GameplayEffects`](#concepts-ge)了！
...
`AbilitySystemComponent`（简称`ASC`）是GAS的核心。它是一个 `UActorComponent` ([`UAbilitySystemComponent`](https://docs.unrealengine.com/en-US/API/Plugins/GameplayAbilities/UAbilitySystemComponent/index.html))，负责处理与系
统的所有交互。任何希望使用 [`GameplayAbilities`](#concepts-ga)、拥有 [`Attributes`](#concepts-a) 或接收[`GameplayEffects`](#concepts-ge) 的 `Actor` 都必
须附有一个 `ASC`。这些对象都存在于并由ASC管理及复制（属性除外，属性由其所属的 [`Attributes`](#concepts-a) 进行复制）。开发者期望但不强制要求
对此进行子类化。

附有 `ASC` 的 `Actor` 被称为 `ASC` 的 `OwnerActor`。 `ASC` 的物理表现形式 `Actor` 称为 `AvatarActor`。在简单的MOBA游戏角色如AI
小兵的情况下， `OwnerActor` 和 `AvatarActor` 可以是同一个 `Actor`。而在玩家控制的英雄角色中，如MOBA游戏，`OwnerActor` 可能是
`PlayerState`， 而 `AvatarActor` 则是英雄的 `Character` 类，这时它们是不同的 `Actor`。大多数情况下，`Actor` 自身会携带 `ASC` 。
如果你的 `Actor` 需要重生，并且在重生之间保持 `Attributes` 或 `GameplayEffects` 的持续性（比如MOBA游戏中的英雄），那么将 `ASC` 放
置在 `PlayerState` 上是最理想的选择。

**注意:** 如果您的 `ASC` 位于 `PlayerState` 上，那么您需要增加 `PlayerState` 的 `NetUpdateFrequency`。在 `PlayerState` 上，
默认值设置得很低，可能导致更改（如 `Attributes` 和 `GameplayTags` ）在客户端上发生之前出现延迟或感知到的滞后现象。 
请务必启用 [`Adaptive Network Update Frequency`](https://docs.unrealengine.com/en-US/Gameplay/Networking/Actors/Properties/index.html#adaptivenetworkupdatefrequency), Fortnite 使用了这项技术/功能。

如果 `OwnerActor` 和 `AvatarActor` 是不同的 `Actor`，它们都应该实现 `IAbilitySystemInterface` 接口。 此接口包含一个必须重写的函
数 `UAbilitySystemComponent* GetAbilitySystemComponent() const`， 该函数返回其 `ASC` 的指针。`ASCs` 通过查找这个接口函数在系统
内部相互交互。

`ASC` 当前活跃的 `GameplayEffects` 是存储在 `FActiveGameplayEffectsContainer ActiveGameplayEffects` 中的。

`ASC` 用于存储已授予的 `Gameplay Abilities（游戏能力）` 在 `FGameplayAbilitySpecContainer ActivatableAbilities` 容器中。每当你
需要遍历 `ActivatableAbilities.Items` 列表时，请确保在循环之前添加 `ABILITYLIST_SCOPE_LOCK()`; 以锁定列表，防止因移除能力而导致的
列表变化。每次使用 `ABILITYLIST_SCOPE_LOCK()`; 都会增加 `AbilityScopeLockCount` 计数器的值，并在退出作用域时减小。
切勿尝试在 `ABILITYLIST_SCOPE_LOCK()`; 锁定范围内移除能力（清除能力功能会内部检查 `AbilityScopeLockCount`，以防止在列表锁定时移除能力）。

<a name="concepts-asc-rm"></a>
### 4.1.1 复制模式
`ASC`（通常指 `Actor` 的 `Gameplay Ability Component` 或者 `Attribute Set Component`）为复制 `GameplayEffects（游戏效果）`、
`GameplayTags（游戏标签）` 和 `GameplayCues（游戏提示）` 定义了三种不同的复制模式：`完整（Full）` 、`混合（Mixed）` 和 
`最小 `（Minimal）`。而 `Attributes（属性）` 的复制则是通过它们各自的 `AttributeSet（属性集）` 来完成。

| 复制模式   | 何时使用                                | 描述                                                                             |
| ------------------ |-------------------------------------|--------------------------------------------------------------------------------|
| `Full`             | 单人游戏                                | 每个 `GameplayEffect` 都会被复制到每个客户端。                                               |
| `Mixed`            | 多人游戏中，玩家控制的角色 `（Actors）`              | `GameplayEffects` 仅被复制到拥有它们的客户端。只有 `GameplayTags` 和 `GameplayCues` 会被复制给所有客户端。 |
| `Minimal`          | 多人游戏中，AI控制的角色`（Actors）` | `GameplayEffects` 不会向任何客户端复制。只有 `GameplayTags` 和 `GameplayCues` 会复制给所有玩家。           |

**注意:** 在 `Mixed` 复制模式下，要求 `OwnerActor` 的 `Owner` 是 `Controller`。默认情况下，`PlayerState` 的 `Owner` 就是 `Controller`，
但 `Character` 的则不是。如果在使用 `Mixed` 复制模式时，`OwnerActor` 并非 `PlayerState`，那么你需要在 `OwnerActor` 上调用
`SetOwner()` 方法并传入一个有效的 `Controller` 对象。

从4.24版本开始，`PossessedBy()` 方法现在会将Pawn的所有权设置为新的 `Controller`。

**[⬆ Back to Top](#table-of-contents)**

<a name="concepts-asc-setup"></a>
### 4.1.2 设置和初始化
`ASCs` 通常在 `OwnerActor` 的构造函数中创建，并且会被明确地标记为需要在网络中复制。 
**这必须在 C++ 中完成**。
...
`ASC` 需要在服务器和客户端上都通过其 `OwnerActor` 和 `AvatarActor` 进行初始化。理想情况下，应在 `Pawn` 的 `Controller` 被设定
（即占有发生后）再进行初始化。对于单人游戏，只需关注服务器端的初始化流程即可

对于玩家控制的角色，其中 `ASC`（Ability System Component）是附着在 `Pawn` 上的，我通常的做法是在服务器端于 `Pawn` 的 `PossessedBy()` 
函数中进行初始化，并在客户端于 `PlayerController` 的 `AcknowledgePossession()` 函数中进行初始化。
...
对于玩家控制的角色，当 `ASC`（Ability System Component）配置在 `PlayerState` 上时，我通常会在服务器端于 `Pawn` 的 `PossessedBy()`
函数中进行初始化，并在客户端于 `Pawn` 的 `OnRep_PlayerState()` 函数中进行初始化。这样做的目的是确保 `PlayerState` 在客户端已存在并
得到同步。
...
如果收到错误信息 `LogAbilitySystem: Warning: Can't activate LocalOnly or LocalPredicted ability %s when not local!`，则表明
您尚未在客户端上初始化 `ASC`（Ability System Component）。确保 `ASC`（Ability System Component）被初始化。

**[⬆ Back to Top](#table-of-contents)**

<a name="concepts-gt"></a>
### 4.2 Gameplay Tags
[`FGameplayTags`](https://docs.unrealengine.com/en-US/API/Runtime/GameplayTags/FGameplayTag/index.html) 是遵循 `Parent.Child.Grandchild...` 这种层级命名方式的标签，它们在 `GameplayTagManager` 中注册。这些标签
对于分类和描述对象状态极为有用。例如，如果一个角色被眩晕了，我们可以在眩晕持续期间给它附加一个 `State.Debuff.Stun` 的 `GameplayTag` 
来表示这一状态。

你会发现自己常常用 `GameplayTags` 来替代过去使用布尔值或枚举处理的情况，并通过布尔逻辑来判断对象是否拥有特定的 `GameplayTags`。

在给对象赋予标签时，我们通常会将这些标签添加到对象的 `ASC`（如果有）上，以便 `GAS`（Gameplay Ability System）能够与这些标签互动。
`UAbilitySystemComponent` 实现了 `IGameplayTagAssetInterface` 接口，提供了访问其拥有的 `GameplayTags` 的功能方法。

多个 `GameplayTags` 可以存储在一个 `FGameplayTagContainer` 中。相较于使用 `TArray<FGameplayTag>`，使用 `GameplayTagContainer` 
更为可取，因为 `GameplayTagContainer` 提供了一些效率优化。尽管标签本质上是标准的 `FNames`，但如果项目设置中启用了 `Fast Replication`，
它们可以高效地被打包在 `FGameplayTagContainers` 中以进行复制。`Fast Replication` 要求服务器和客户端具有相同的` GameplayTags` 列表。
这通常不会成为问题，因此建议启用此选项。此外，`GameplayTagContainers` 也能提供 `TArray<FGameplayTag>` 以便于遍历操作。

存储在 `FGameplayTagCountContainer` 中的 `GameplayTags` 具有一个 `TagMap`，用于记录该 `GameplayTag` 的实例数量。
即使 `FGameplayTagCountContainer` 中仍然包含某个 `GameplayTag`，但其 `TagMapCount` 可能为零。如果你在调试过程中发现一个 `ASC` 仍
旧含有某个 `GameplayTag`，但相关功能却未按预期工作，这可能是原因所在。诸如 `HasTag()`、`HasMatchingTag()` 之类的函数会检查 `TagMapCount`，
如果该 `GameplayTag` 不存在或其 `TagMapCount` 为零，则这些函数会返回 `false`。

`GameplayTags` 需要预先在 `DefaultGameplayTags.ini` 文件中定义。虚幻编辑器在项目设置中提供了一个界面，允许开发者管理 `GameplayTags`，
无需手动编辑 `DefaultGameplayTags.ini` 文件。`GameplayTag` 编辑器支持创建、重命名、查找引用及删除 `GameplayTags` 等功能，
极大地方便了开发者对标签的管理。

![在项目设置中编辑GameplayTags](https://github.com/tranek/GASDocumentation/raw/master/Images/gameplaytageditor.png)

在编辑器中搜索 `GameplayTag` 引用时，会弹出熟悉的 `Reference Viewer`（引用查看器）图表，展示所有引用该 `GameplayTag` 的资源。
但是，这并不会显示任何C++类中对 `GameplayTag` 的引用。

重命名 `GameplayTags` 时，系统会创建一个重定向，以便仍引用原始 `GameplayTag` 的资产能自动指向新 `GameplayTag`。然而，如果可能的话，
我更倾向于采用另一种做法：先创建一个新的 `GameplayTag`，手动更新所有对旧标签的引用至新标签，然后再删除旧的 `GameplayTag`，以此来避免产
生重定向。

除了启用 `Fast Replication` 之外，`GameplayTag` 编辑器还提供了一个选项，可以填写常见复制的 `GameplayTags`，以便进一步优化它们的网络
传输效率。这通常涉及预填充或标记那些频繁在网络间同步的标签，从而使引擎能够更高效地处理这些标签的复制过程。

当 `GameplayTags` 通过 `GameplayEffect` 添加时，它们会被复制。而 `ASC` 允许你添加不被复制的 `LooseGameplayTags`，这类标签需要手动
管理。示例项目为 `State.Dead` 中使用了一个 `LooseGameplayTag`，以便拥有它的客户端能在其生命值降至零时立即作出响应。重生时需手动将
`TagMapCount` 重置为零。仅在处理 `LooseGameplayTags` 时才需手动调整 `TagMapCount`。相比之下，使用
`UAbilitySystemComponent::AddLooseGameplayTag()` 和` UAbilitySystemComponent::RemoveLooseGameplayTag()` 函数比直接手动调整
`TagMapCount`更可取。
...
对于高级的 `GameplayTag` 操作，如获取父级或子级 `GameplayTags`，可以查阅 `GameplayTagManager` 提供的函数。要访问 `GameplayTagManager`，
需包含 `GameplayTagManager.h` 头文件，并通过 `UGameplayTagManager::Get().FunctionName` 的方式来调用相关功能。实际上，
`GameplayTagManager` 将 `GameplayTags` 以关系节点（如父、子等）的形式存储，这相比于持续的字符串操作和比较能显著提升处理速度。

`GameplayTags` 和 `GameplayTagContainers` 可以拥有一个可选的 `UPROPERTY` 指定符 `Meta = (Categories = "GameplayCue")`，该指定
符在蓝图中过滤标签，仅显示那些父标签为 `GameplayCue的GameplayTags`。当你明确知道某个 `GameplayTag` 或 `GameplayTagContainer` 变量
仅应用于 `GameplayCues` 时，这个功能特别有用，能够帮助组织和区分不同用途的标签。

另外，还有一个独立的结构体叫做 `FGameplayCueTag`，它封装了 `FGameplayTag`，并且在蓝图中会自动过滤 `GameplayTags`，仅展示那些父标签
为 `GameplayCue` 的标签。这种方式提供了更加针对性和便捷的管理，尤其适合专门处理 `GameplayCues` 的场景。

若想在函数的 `GameplayTag` 参数上应用过滤，可以使用 `UFUNCTION` 修饰符 `Meta = (GameplayTagFilter = "GameplayCue")`。然而，函数
中的 `GameplayTagContainer` 参数目前不支持直接过滤。如果你想自定义引擎以允许这种过滤，可以参考
`EnginePluginsEditorGameplayTagsEditorSourceGameplayTagsEditorPrivateSGameplayTagGraphPin.cpp` 中的 `SGameplayTagGraphPin::ParseDefaultValueData()`
函数实现。该函数通过调用 `FilterString = UGameplayTagsManager::Get().GetCategoriesMetaFromField(PinStructType)`;来获取过滤字
符串，并在 `SGameplayTagGraphPin::GetListContent()` 中将此 `FilterString` 传递给 `SGameplayTagWidget` 来实现过滤功能。 相对应地，
在 `EnginePluginsEditorGameplayTagsEditorSourceGameplayTagsEditorPrivateSGameplayTagContainerGraphPin.cpp `文件中，关于
`GameplayTagContainer` 的类似函数并未检查元字段属性，也没有传递过滤条件。如果希望对 `GameplayTagContainer` 参数也实现类似过滤，你可
能需要在此基础上进行相应的修改和扩展。

示例项目广泛采用了 `GameplayTags`。

**[⬆ Back to Top](#table-of-contents)**

<a name="concepts-gt-change"></a>
### 4.2.1 响应 GameplayTag 的变化
`ASC`（UAbilitySystemComponent）提供了一个委托（delegate），用于在 `GameplayTags` 被添加或移除时触发。 这个委托接受一个 `EGameplayTagEventType` 
枚举作为参数，该枚举可以指定仅在 `GameplayTag` 被添加/移除时触发，或是 `GameplayTag` 的 `TagMapCount` 有任何变化时触发。这样设计使得
开发者能够精确地响应标签状态的改变，执行相应的逻辑处理。

```c++
AbilitySystemComponent->RegisterGameplayTagEvent(FGameplayTag::RequestGameplayTag(FName("State.Debuff.Stun")), EGameplayTagEventType::NewOrRemoved).AddUObject(this, &AGDPlayerState::StunTagChanged);
```

回调函数包含一个参数，用于传递发生变更的 `GameplayTag` 以及该标签的新 `TagCount`（标签计数）。
```c++
...
#### 4.3.1 Attribute 定义
`Attributes` 是由结构体 [`FGameplayAttributeData`](https://docs.unrealengine.com/en-US/API/Plugins/GameplayAbilities/FGameplayAttributeData/index.html) 定义的浮点数值。这些数值可以代表从角色的血量、等级到药水的使用次数等任何与游
戏性相关的数值。如果是一个属于Actor的游戏性相关数值，应考虑使用 `Attribute` 来表示。通常情况下，Attributes只应由 [`GameplayEffects`](#concepts-ge) 
修改，以便 `ASC` 能够 [predict](#concepts-p) 这些变化。

`Attributes` 由 [`AttributeSet`](#concepts-as) 定义并存于其中。`AttributeSet` 负责复制那些被标记为需要复制的 `Attributes`。有关如何定义 
`Attributes` 的详细信息，请参阅关于 [`AttributeSets`](#concepts-as) 的部分。

**技巧:** 如果你不想让某个 `Attribute` 出现在编辑器的 `Attributes` 列表中，可以使用 `Meta = (HideInDetailsView)` 的属性指定符来隐
藏它。

**[⬆ Back to Top](#table-of-contents)**

<a name="concepts-a-value"></a>
#### 4.3.2 BaseValue（基础值）vs BaseValue（基础值）
一个 `Attribute` 由两个值组成 `BaseValue`（基础值）和 `BaseValue`（基础值）。`BaseValue` 代表了该属性的永久性数值，而` CurrentValue`
则是 `BaseValue` 加上来自 `GameplayEffects` 的临时修改值。例如，你的 `Character` 可能有一个移动速度 `Attribute`，其 `BaseValue` 
为每秒600单位。由于目前没有 `GameplayEffects` 影响移动速度，所以 `CurrentValue` 同样为600单位/秒。如果她获得了一个暂时性的50单位/秒
移动速度增益，那么 `BaseValue` 保持不变仍然是600单位/秒，而 `CurrentValue` 则变为600 + 50，总计650单位/秒。当这个移动速度增益效果结
束后，`CurrentValue` 会重新回到600单位/秒的 `BaseValue`。

初学者在使用 `GAS`（Gameplay Ability System）时常会混淆 `BaseValue` 与属性的最大值概念，并尝试将其当作最大值来处理，这是一种错误的做
法。对于可以变化或在技能（abilities）及用户界面（UI）中引用的属性最大值，应当将其视为独立的 `Attributes` 来处理。对于硬编码的最大值和.
最小值，虽然可以通过定义包含 `FAttributeMetaData` 的` DataTable` 来设定这些界限，但Epic对此结构体的注释表明它仍处于“开发中”。详情可
参考 `AttributeSet.h` 文件。为了避免混淆，建议将技能或UI中可能引用的最大值设为单独的 `Attributes`；而对于仅用于限制（clamping）
`Attributes`的硬编码最大和最小值，则应在 `AttributeSet` 中定义为硬编码的浮点数。关于属性的限制，对于 `CurrentValue` 的更改，可以在
`PreAttributeChange()` 部分讨论；而对于由 `GameplayEffects` 引起的 `BaseValue` 更改，则在 `PostGameplayEffectExecute()`部分探讨。

对 `BaseValue` 的永久性改动源自 `Instant`（即时）`GameplayEffects`，而具有 `Duration`（持续时间）和 `Infinite`（无限持续）的
`GameplayEffects` 则会影响 `CurrentValue`。周期性（Periodic）的 `GameplayEffects` 会被当作即时 `GameplayEffects` 处理，同样会改
变 `BaseValue`。
...
某些 `Attributes` 充当临时值的占位符，旨在与实际 `Attributes` 互动，这类特殊的属性被称为 `Meta Attributes`（元属性）。例如，我们常
将伤害定义为一个 `Meta Attribute`。伤害不是直接由 `GameplayEffect` 改变我们的生命值 `Attribute`，而是先使用名为“伤害”的 
`Meta Attribute`作为中间变量。这样，伤害值就可以在 [`GameplayEffectExecutionCalculation`](#concepts-ge-ec) 中通过增益和减益效果进行调整，并且在
`AttributeSet` 中进一步操作，比如先从当前护盾 `Attribute` 中扣除伤害值，之后再从生命值 `Attribute` 中扣除剩余的伤害。
伤害 `Meta Attribute` 在不同的 `GameplayEffects` 之间没有持久性，每次新的效果都会覆盖之前的值。通常情况下，`Meta Attributes` 不会
在网络间进行同步复制。

`Meta Attributes` 为诸如伤害和治疗等操作提供了一个良好的逻辑分离，区分了“我们造成了多少伤害？”和“我们如何处理这部分伤害？”。这种逻辑上
的分离意味着我们的 `Gameplay Effects`（游戏效果）和 `Execution Calculations`（执行计算）不需要了解目标是如何处理伤害的细节。继续以伤
害为例，`Gameplay Effect` 确定了伤害的数量，然后由 `AttributeSet` 决定如何处理这笔伤害。并非所有角色都拥有相同的 `Attributes`，特别
是当你使用子类化的 `AttributeSets` 时。基类 `AttributeSet` 可能只包含一个生命值 `Attribute`，但子类化的 `AttributeSet` 可能会增加
一个护盾 `Attribute`。拥有护盾 `Attribute` 的子类化 `AttributeSet` 会与基类 `AttributeSet` 以不同方式分配所受的伤害。这样的设计提
高了系统的灵活性和可扩展性，使得游戏逻辑更为清晰和模块化。

尽管使用 `Meta Attributes` 是一种良好的设计模式，但它们并不是强制性的。如果你对所有伤害实例都只使用一种 `Execution Calculation`（执行计算），
并且所有角色共用一个 `Attribute Set`（属性集）类，那么你可能直接在 `Execution Calculation` 内部进行伤害分配至生命值、护盾等操作，并
直接修改这些 `Attributes` 也是可行的。这样做你可能会牺牲一定的灵活性，但如果这对你的项目来说是可以接受的，那么这种方式也是可以的。简而言
之，是否采用 `Meta Attributes` 取决于你的项目需求、系统的复杂度以及对未来扩展性的考量。

**[⬆ Back to Top](#table-of-contents)**

<a name="concepts-a-changes"></a>
#### 4.3.4 响应属性更改
要监听 `Attribute` 变化以更新UI或其他游戏玩法，可以使用 `UAbilitySystemComponent::GetGameplayAttributeValueChangeDelegate(FGameplayAttribute Attribute)`
函数。此函数返回一个委托（delegate），你可以绑定到该委托上，每当 `Attribute` 发生变化时，它会自动被调用。
该委托提供一个 `FOnAttributeChangeData` 参数，包含了 `NewValue`（新值）、`OldValue`（旧值）以及 `FGameplayEffectModCallbackData`。
...
示例项目中，在 `GDPlayerState` 上注册了 `Attribute` 值变更的监听委托，以便在健康值降到零时刷新HUD（抬头显示器）并作出响应处理玩家死亡
的情况。

示例项目中包含了一个自定义的蓝图节点，该节点将上述功能封装为了一个 `ASyncTask`。这一异步任务在 `UI_HUD` UMG界面 `widget` 中被用来实时
更新生命值、法力值和耐力值。这个 `ASyncTask` 将持续运行，直到手动调用 `EndTask()` 方法终止，我们在UMG界面 `widget` 的` Destruct`（销毁）
事件中执行了这一操作。更多细节可查看 `AsyncTaskAttributeChanged.h/cpp` 源文件。
...
要创建一个其值部分或全部来源于一个或多个其他 `Attributes` 的 `Derived Attribute`（派生属性），可以使用带有一个或多个基于 `Attribute`
的或 [`MMC`](#concepts-ge-mmc)（Modified Modifier Magnitude Calculator，修正修饰符幅度计算器）[`Modifiers`](#concepts-ge-mods)
（修饰符）的 `Infinite`（无限持续）`GameplayEffect`。当依赖的 `Attribute` 更新时，`Derived Attribute` 会自动随之更新。

对于 `Derived Attribute`（衍生属性）上的所有 `Modifiers`（修饰符），其最终计算公式与 `Modifier Aggregators`（修饰符聚合器）所使用
的公式相同。如果需要按照特定顺序执行计算，那么应将所有计算逻辑置于一个 `MMC`（Modified Modifier Magnitude Calculator，修改后的修饰符幅度计算器）内部完成

```
((CurrentValue + Additive) * Multiplicitive) / Division
```

**注意:** 如果在PIE（Play In Editor，编辑器内播放）模式下使用多个客户端进行测试，你需要在编辑器偏好设置中禁用 `Run Under One Process`
选项。否则，当除第一个客户端以外的其他客户端上的独立 `Attributes` 更新时，`Derived Attributes`（衍生属性）将无法得到更新。

在这个例子中，我们应用了一个 `Infinite`（无限持续）的 `GameplayEffect`，用来根据公式 `TestAttrA = (TestAttrA + TestAttrB) * (2 * TestAttrC)`
从 `Attributes TestAttrB` 和 `TestAttrC` 派生出 `TestAttrA` 的值。每当 `TestAttrB` 或 `TestAttrC` 中的任何一个属性更新其值时，
`TestAttrA` 都会自动重新计算其自身的值。
...
`AttributeSet` 负责定义、保存及管理 `Attributes` 的变化。开发者应当从 [`UAttributeSet`](https://docs.unrealengine.com/en-US/API/Plugins/GameplayAbilities/UAttributeSet/index.html) 类进行派生。在 `OwnerActor` 的构造函
数中创建一个 `AttributeSet` 会自动将其注册到相应的 `ASC`（Ability System Component，能力系统组件）上。这一点必须在C++代码中实现。

**[⬆ Back to Top](#table-of-contents)**

<a name="concepts-as-design"></a>
#### 4.4.2 Attribute Set 设计
一个 `ASC`（Ability System Component，能力系统组件）可以拥有一个或多个 `AttributeSets`。`AttributeSets` 带来的内存开销微乎其微，
因此决定使用多少个 `AttributeSets` 主要取决于开发者的组织结构决策，以达到代码管理和逻辑清晰的目的。

在游戏中，每个 `Actor` 都共用一个大型的、综合性的 `AttributeSet` 是可行的。你可以按需使用其中的属性而忽略那些未使用的属性。

或者，你可以选择拥有多个 `AttributeSet`，每个集合代表一类属性，根据需要有选择性地将它们添加到你的 `Actors` 中。例如，你可以为与生命值
相关的属性创建一个 `AttributeSet`，为与魔法值相关的属性创建另一个 `AttributeSet` ，等等。在MOBA游戏中，英雄可能需要魔法值，而小兵则不
需要。因此，英雄会获得魔法值的 `AttributeSet`，而小兵则不会。

此外，可以通过子类化 `AttributeSets` 作为另一种方式来有选择性地决定一个 `Actor` 具有哪些 `Attributes`。在内部，`Attributes`通过
`AttributeSetClassName.AttributeName` 的形式来引用。当你子类化一个 `AttributeSet` 时，来自父类的所有 `Attributes` 仍会以前缀形式
保留父类的名称。这种方法允许你在不改变原有属性定义的基础上，为特定类型的 `Actors` 添加额外的属性，从而实现更细粒度的定制。

尽管你可以在一个 `ASC`（Ability System Component）上拥有多个 `AttributeSet`，但不应该在一个 `ASC` 上有超过一个来自相同类别的 `AttributeSet`。
如果有来自同一类的多个 `AttributeSet`，系统将无法确定应该使用哪一个，从而可能导致意外的行为，通常它只会选择其中一个来使用。为了避免这种
情况，确保每个ASC上每个 `AttributeSet` 类只有一个实例。

<a name="concepts-as-design-subcomponents"></a>
##### 4.4.2.1 子组件与独立属性
当你的 `Pawn`（游戏实体）上有多个可受损部件，比如单独可破坏的盔甲片时，如果已知一个 `Pawn` 最多能有多少个可受损部件，我建议在单个 `AttributeSet`
中创建相应数量的健康 `Attributes` ——如 `DamageableCompHealth0`、`DamageableCompHealth1` 等，来表示这些可受损部件的逻辑“槽位”。在
你的可受损部件类实例中，分配一个槽位编号 `Attribute`，该属性可被 `GameplayAbilities` 或[`Executions`](#concepts-ge-ec)读取，以便知道要对哪个
`Attribute` 施加伤害。拥有少于最大数量或零个可受损部件的 `Pawns` 也是可以的。仅仅因为 `AttributeSet` 中存在某个 `Attribute`，并不意
味着你必须使用它。未使用的 `Attributes` 占用的内存量微不足道。

如果子组件各自需要很多 `Attributes`，或者子组件的数量可能是无限的，子组件可以拆卸并被其他玩家使用（例如武器），或者由于任何其他原因导致
上述方法不适合你，我建议不再使用 `Attributes`，转而在组件上直接存储普通的浮点数。
请参阅[Item Attributes](#concepts-as-design-itemattributes)部分。

<a name="concepts-as-design-addremoveruntime"></a>
##### 4.4.2.2 在运行时添加和移除AttributeSets
`AttributeSets` 可以在运行时从ASC中添加或移除；然而，移除 `AttributeSets` 可能存在风险。例如，如果在一个客户端上先于服务器移除了某个
`AttributeSet`，然后该 `Attribute` 的值变化被复制到客户端时，该 `Attribute` 将找不到对应的 `AttributeSet`，从而导致游戏崩溃。
...
> 1. 在项目上使用普通浮点数 (**推荐**)
> 1. 在物品上分离的 `AttributeSet`
> 1. 在物品上分离的 `ASC`

<a name="concepts-as-design-itemattributes-plainfloats"></a>
###### 4.4.2.3.1 物品上的普通浮点数属性
不使用 `Attributes`，而是在物品类实例上直接存储简单的浮点数值。《堡垒之夜》和 [GASShooter](https://github.com/tranek/GASShooter) 就是采用这种方式处理枪械弹药的。对于一
把 枪，可以将最大弹夹容量、当前弹夹内的子弹数量、备用弹药等信息直接作为复制品浮点数（使用 `COND_OwnerOnly` 条件）存储在枪实例上。如果多
把武 器共享备用弹药，则可以将备用弹药移到角色上，作为一个共享弹药 `AttributeSet` 中的 `Attribute`（重装能力可以使用 `Cost GE` 从备用
弹药中 抽 取到枪的浮点数弹夹中）。由于没有使用 `Attributes` 来表示当前弹夹中的弹药，你需要覆盖 `UGameplayAbility` 中的一些函数，以便
检查和应 用针对枪械上浮点数的成本。在赋予能力时，将枪械设为 [`GameplayAbilitySpec`](https://github.com/tranek/GASDocumentation#concepts-ga-spec) 中的 `SourceObject` 意味着在能力内部可以访
问授予该能 力的枪械。

为了防止在自动射击过程中枪械因回传弹药数量而导致本地弹药数量被覆盖，当玩家具有 `IsFiring GameplayTag` 时，在 `PreReplication()` 中禁
用复制。实质上，这里你是在进行自己的局部预测处理。
...
1. 需要对 `UGameplayAbility` 的关键函数进行覆写，以便检查及根据枪械的浮点数值应用弹药消耗

<a name="concepts-as-design-itemattributes-attributeset"></a>
###### 4.4.2.3.2 物品上的 `AttributeSet`
在物品上使用独立的 `AttributeSet`，并在将其添加到玩家的库存时一并添加到玩家的 `ASC` 中，这种方法可行，但也存在一些重大限制。
我在[GASShooter](https://github.com/tranek/GASShooter)的早期版本中为武器弹药实现了这一功能。武器将其属性（如最大弹夹容量、当前弹夹内子弹数量、备用弹药等）存储在一个位于
武器类上的 `AttributeSet` 中。如果多把武器共享备用弹药，你可以将备用弹药移动到角色上的共享弹药 `AttributeSet` 中。当服务器上将武器添
加到玩家的库存时，武器会将其 `AttributeSet` 添加到玩家 `ASC` 的 `SpawnedAttributes` 中。随后，服务器会将此变更复制到客户端。如果武器
从库存中移除，它也会从 `ASC::SpawnedAttributes` 中移除其 `AttributeSet`。
尽管这种方法可以实现属性管理，但它的问题包括但不限于：
* 网络和性能开销：频繁的添加和移除操作可能导致网络同步负担加重，以及玩家ASC管理的复杂度提高。
* 潜在的同步问题：如之前提到的，不同步的移除操作可能导致崩溃或数据不一致。
* 设计复杂度：需要更复杂的逻辑来管理物品与玩家ASC之间的关系，尤其是在处理多人交互和网络环境下的情况。

当 `AttributeSet` 存在于非 `OwnerActor`（比如武器）上时，最初在 `AttributeSet` 中可能会遇到一些编译错误。解决方法是在 `BeginPlay()`
中而非构造函数中构建 `AttributeSet`，并且在武器上实现 `IAbilitySystemInterface`（当武器加入玩家库存时设置指向ASC的指针）。
...
你可以通过查看[旧版GASShooter](https://github.com/tranek/GASShooter/tree/df5949d0dd992bd3d76d4a728f370f2e2c827735)的这个较旧版本来实际了解这一实现方式。
